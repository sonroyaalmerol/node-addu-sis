...
const { fetch: _fetch, CookieJar } = require('node-fetch-cookies-rn');
    ...
    const prevPage = async (sem, year, cookieJar) => {
      try {
        ...
        if (page) {
          ...
          if (_sem && _year) {
            await prevPage(_sem.value, _year.value, cookieJar)
          }
        }
      } catch (err) {
        return null
      ...
      try {
        ...
        if (page) {
          ...
          if (_sem && _year && _period) {
            await prevPage(_sem.value, _year.value, _period.value, cookieJar)
          }
        }
      } catch (err) {
        return null
