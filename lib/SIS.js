...
const { fetch, CookieJar } = require('node-fetch-cookies-rn');
...
class SIS {
  ...
  async _request(url) {
    ...
    try {
      ...
      var x = await fetch(cookieJar, `${this.#url}?destination=${url}`, { 
        method: 'POST',
  ...
  async checkAuth() {
    ...
    if (dom.querySelector("#block-block-3 > div > div > div.sis_profile_links > ul > li:nth-child(1) > a")) {
      return true
    } else {
      return false
    }
  ...
  async getGrades() {
    ...
    const prevPage = async (sem, year, cookieJar) => {
      try {
        var page = await fetch(cookieJar, `${this.#url}/grades/${sem}/${year}`, { 
          method: 'GET'
        })
        if (page) {
  ...
  async getBalance() {
    ...
    const prevPage = async (sem, year, period, cookieJar) => {
      try {
        var page = await fetch(cookieJar, `${this.#url}/balance/${sem}/${year}/${period}`, { 
          method: 'GET'
        })
        if (page) {
          var body = await page.text()
  ...
  async searchClassOfferings(classCode) {
    ...
    try {
      var x = await fetch(page.cookieJar, `${this.#url}/search_subjects`, { 
        method: 'POST',
  ...
  async searchPrereg(classCode) {
    ...
    var z = await fetch(page.cookieJar, `${this.#url}/preregistration/add/search`, { 
      method: 'GET'
    })
    ...
    try {
      var x = await fetch(page.cookieJar, `${this.#url}/preregistration/add/search`, { 
        method: 'POST',
        body: qs.stringify(input),
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
  ...
  async addPrereg(classCode, session) {
    ...
    try {
      var x = await fetch(session.cookieJar, `${this.#url}/preregistration/add/result`, { 
        method: 'POST',
        body: qs.stringify(input),
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
      ...
      var y = await fetch(session.cookieJar, `${this.#url}/preregistration/add/confirm`, { 
        method: 'POST',
        body: qs.stringify(input),
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
  ...
  async dropPrereg(id) {
    ...
    try {
      var x = await fetch(page.cookieJar, `${this.#url}/preregistration`, { 
        method: 'POST',
        body: qs.stringify(input),
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
      ...
      var y = await fetch(page.cookieJar, `${this.#url}/preregistration/drop/confirm`, { 
        method: 'POST',
        body: qs.stringify(input),
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
