...
class SIS {
  ...
  constructor({ cache }) {
    this.#url = (() => ["https://sis.addu.edu.ph", "https://sis2.addu.edu.ph"][Math.floor(Math.random() * Math.floor(2))])
    this.#username = ''
    this.#password = ''
    this.#cookies = ''
    this.#poll = null
    ...
    this._resetCookiesTimer = () => {
      if (cache) {
        return this.#cache.ttl(`${this.#username}$${this.#password}$cookies`, 180)
      } else {
        return null
      }
    }
  ...
  async login(username, password) {
    var rawCookies = this._getCache('cookies')
    if (!rawCookies) {
      try {
        ...
        rawCookies = await loginPage.cookies()

        loginPage.close()

        this._setCache('cookies', rawCookies, 180)
      } catch (err) {
        throw new Error('AdDU SIS Server Timeout.')
      }
    } else {
    ...
    this.#cache.on('expired', (key) => {
      if (key === `${this.#username}$${this.#password}$cookies`) {
        this.#cookies = ''
      }
    })
  }
  ...
  async searchClass(classCode) {
    ...
    if (!toReturn) {
      ...
      var cookies = this._getCache('cookies')
      var page = null
      ...
      try {
        ...
        page = await searchPage.content()
      } catch (err) {
        throw new Error('AdDU SIS Server Timeout.')
      }
      if (page) {
        const data = tabletojson.convert(page)[0]
        searchPage.close()
